...
def runEpisode(agent, environment, discount, decision, display, message, pause, episode):
    #這邊的environment是指GUI場面上的所有state包含grid
    ...
    while True:
        ...
        action = decision(state)#會跑到502行那邊的class agent 的class agent那邊
        if action == None:
        ...
        returns += reward * totalDiscount#這邊多一個totalDiscount表示
        totalDiscount *= discount#discount會做每一次的修正，不過在座transition model的時候比較需要注意
